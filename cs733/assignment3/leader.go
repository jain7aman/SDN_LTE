...
func (sm *RaftServer) leader() State {
	...
	matchIndex[sm.Id] = len(sm.Log) - 1
	sm.VotedFor = -1
	//Setup the hearbeat timer TIMEOUT event
	sm.SendChannel <- Alarm{Time: sm.Configuration.HeartbeatTimeout} //sleep for RANDOM time
	...
	for event := range sm.ReceiveChannel {
			...
			for i := uint(0); i < sm.N; i++ {
				if i != sm.Id {
					if int(nextIndex[i]) <= (len(sm.Log) - 1) { //receiver log is not uptodate
						...
						sm.SendChannel <- Send{i, AppendEntriesReqEvent{Entries: newEntries, LeaderCommit: sm.CommitIndex, LeaderId: sm.Id, PrevLogIndex: int(nextIndex[i]) - 1, PrevLogTerm: prevLogTerm, Term: sm.Term}}
					} else {
						// sends the empty heartbeat
						sm.SendChannel <- Send{i, AppendEntriesReqEvent{Entries: make([]LogEntry, 0), LeaderCommit: sm.CommitIndex, LeaderId: sm.Id, PrevLogIndex: int(nextIndex[i]) - 1, PrevLogTerm: prevLogTerm, Term: sm.Term}}
					}
				}
			}
		...
		case AppendEntriesRespEvent:
			...
			if msg.Success == false {
				...
				tempMatchIndex := make([]int, len(matchIndex))
